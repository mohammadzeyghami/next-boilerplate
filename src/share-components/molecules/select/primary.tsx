...
import { useSearchFilter } from "@/share-components/hooks/useSearchFilter";
...
function SelectPrimary({
  ...
  const mergedOptions = useMemo(() => {
    ...
    return Array.from(map, ([optionValue, optionLabel]) => ({
      value: optionValue,
      label: optionLabel,
    }));
  }, [createdOptions, deletedIds, editedLabels, options]);

  const shouldLocalSearch =
    searchable && (localSearchEnabled || !hasRemoteSearch);
  ...
  const selectedOption = useMemo(() => {
    return mergedOptions.find((opt) => opt.value === (value ?? "")) ?? null;
  }, [mergedOptions, value]);
  ...
  useLayoutEffect(() => {
    ...
    const labelTexts = mergedOptions
      ...
      .map((text, idx) =>
        text.trim() ? text : (mergedOptions[idx]?.value ?? ""),
      )
      .filter(Boolean);
    ...
    setMinWidth(
      Math.ceil(maxTextWidth + paddingLeft + paddingRight + iconSpace),
    );
  ...
  const handleCreateComplete = useCallback(
    async (info?: { id?: string; label?: string }) => {
      ...
      const optionLabel = info.label?.trim() || id;

      setCreatedOptions((prev) =>
        prev.some((opt) => opt.value === id)
          ? prev
          : [{ value: id, label: optionLabel }, ...prev],
      );
  ...
  const showAddAtBottom =
    Boolean(create) && filteredOptions.length > 0 && !showCreateFromNoResults;
    ...
    <div className="grid gap-2" style={minWidth ? { minWidth } : undefined}>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="max-h-72 overflow-y-auto w-(--radix-popper-anchor-width) min-w-(--radix-popper-anchor-width)">
          {topControls ? (
            <div className="sticky top-0 z-10 grid gap-2 bg-popover px-2 py-1.5 pb-2">
              {searchable ? (
            ...
            >
              {filteredOptions.map((opt) => (
                <DropdownMenuRadioItem
                  key={opt.value}
                  value={opt.value}
                  className="group/opt flex items-center justify-between gap-1"
                >
      ...
      {editState.open && create?.editRender
        ? create.editRender({
            id: editState.id,
